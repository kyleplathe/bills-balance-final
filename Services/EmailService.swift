...
final class EmailService {
    ...
    init(
        ...
        self.apiKey = ProcessInfo.processInfo.environment["RESEND_API_KEY"] ?? ""
    ...
    private func buildWeeklySnapshot(referenceDate: Date) async throws -> WeeklyFinancialSnapshot {
        ...
        let dueBills: [WeeklyDueBillRow] = try await database.client
            .from("bills")
        ...
        let accountRows: [AccountBalanceRow] = try await database.client
            .from("accounts")
            .select("balance")
            .execute()
            .value
