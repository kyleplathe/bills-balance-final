...
final class LedgerService {
    ...
    func processBillPayment(for bill: Bill) async throws {
        ...
        if bill.recurrence.lowercased() != "none" {
            ...
            try await createBill(nextBill)
        }
    }

    private func createBill(_ bill: Bill) async throws {
...
private struct BillPaymentRPCParams: Encodable {
    let billID: UUID
    let accountID: UUID
...
private struct BillInsert: Encodable {
    let id: UUID
    let name: String
    let amount: Decimal
    let dueDate: Date
